...
def generate_receipt(request, order_id):
    ...
    p.drawString(100, 800, "Farmers Market Receipt")
    p.drawString(100, 780, f"Receipt #: {receipt.receipt_number}")
    p.drawString(100, 760, f"Order #: {order.id}")
    p.drawString(100, 740, f"Client: {order.client.name}")
    p.drawString(100, 720, f"Amount: {order.total_amount}")
...
def confirm_received(request, order_id):
    ...
    if order.status == "Delivered":
        messages.success(
            request,
            f"Order #{order.id} confirmed received."
        )
